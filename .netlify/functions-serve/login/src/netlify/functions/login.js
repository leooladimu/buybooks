...
var handler = async (event, _context) => {
  ...
  const user = db.data.users.find((user2) => user2.email === email && user2.password === hashedPassword);
  return user ? {
    statusCode: 200,
    body: JSON.stringify(__spreadProps(__spreadValues({}, user), { password: void 0 }))
  } : {
    statusCode: 404,
