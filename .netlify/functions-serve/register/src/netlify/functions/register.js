...
var hashOf = (password) => (0, import_crypto.createHmac)("sha256", import_process.env.PASSWORD_SECRET).update(password).digest("hex");
var uid = () => (0, import_crypto.randomBytes)(256).toString("hex");
...
var handler = async (event, _context) => {
  ...
  const doesUserExist = db.data.users.some((user) => user.email === email && user.password === hashedPassword);
  if (doesUserExist) {
    return {
      statusCode: 406,
  ...
  db.data.users.push(__spreadProps(__spreadValues({}, userData), {
